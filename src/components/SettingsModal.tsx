...
const DataAction = ({ icon: Icon, title, description, onClick, highlight = false }: { icon: any, title: string, description: string, onClick: () => void, highlight?: boolean }) => (
  <button 
    onClick={onClick}
    className={cn(
...
const ConfirmModal = ({ title, description, onConfirm, onCancel, danger = false }: { title: string, description: string, onConfirm: () => void, onCancel: () => void, danger?: boolean }) => (
  <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onCancel} className="absolute inset-0 bg-background/60 backdrop-blur-sm" />
    <motion.div 
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={onCancel} className="p-3 bg-muted rounded-2xl font-bold text-sm tracking-tight hover:bg-muted/80 transition-colors">Cancel</button>
        <button 
          onClick={onConfirm}
          className={cn(
